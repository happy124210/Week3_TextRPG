...
namespace TextRPG_Team25
{
    internal class Battle
    {
        ...
        {
            int battleHp = player.hp; //전투에서쓰는 HP따로 선언
        ...
        if (randomDmg == 1)
                {
                    if (randomDmg == 1) { double i = Monster.attack; i = i * 90 / 100; double enermyDmg = Math.Round(i); }     //10퍼센트 확률로 90퍼센트의 데미지 반올림
                    else if (randomDmg == 10)
                    {
                        if (randomDmg == 1) { double i = Monster.attack; i = i * 90 / 100; double enermyDmg = Math.Round(i); } //10퍼센트의 확률로 110퍼센트의 데미지 반올림


                        if (EnermyList[i].EnermyDead == true) { randomDmg = 0 }
